"""
Recompute embeddings for ALL rows
(should be called only when the embedding model changed).
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                ...
                "DROP COLUMN IF EXISTS embedding_q_new;"
            )
        )
        conn.execute(
            text(
                f"ALTER TABLE faq_items "
                f"ADD COLUMN embedding_q_new VECTOR({dim});"
            )
        )
...
def _reembed_all_rows_into_new() -> int:
    ...
    stmt = text(
        "UPDATE faq_items SET embedding_q_new = :vec WHERE id = :id"
    ).bindparams(
    ...
    with Session(engine) as session:
        rows: list[FAQItem] = list(
            session.execute(select(FAQItem)).scalars().all()
        )
        if not rows:
            return 0
...
def _swap_columns() -> None:
    ...
    with engine.begin() as conn:
        conn.execute(
            text("ALTER TABLE faq_items " "DROP COLUMN IF EXISTS embedding_q;")
        )
        conn.execute(
            text(
                "ALTER TABLE faq_items "
                "RENAME COLUMN embedding_q_new TO embedding_q;"
            )
        )
...
def main() -> None:
    ...
    _swap_columns()
    print(
        f"Re-embedded {count} rows and switched embedding_q "
        f"to dim={settings.embedding_dim}."
    )
